<template>
  <div class="max-w-4xl">
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
      <div class="mb-6">
        <h2 class="text-2xl font-bold text-gray-900">{{ isEdit ? 'Edit' : 'Create' }} Gift Type</h2>
        <p class="text-gray-600 mt-1">{{ isEdit ? 'Update gift type information' : 'Add a new gift type' }}</p>
      </div>
      ...
      <form @submit.prevent="submit" class="space-y-6">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <input 
              v-model="form.slug" 
              class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
              placeholder="gift-type-slug (auto-generated)"
            />
        ...
        <div>
          ...
          <textarea 
            ...
            rows="4"
            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
            placeholder="Gift type description"
          ></textarea>
        </div>

        <div>
          <label class="block text-sm font-medium text-gray-700 mb-2">Gift Type Image</label>
          
          <!-- Image Preview -->
          <div v-if="imagePreview" class="mb-4">
            <img 
              :src="imagePreview" 
              alt="Gift type preview" 
...
// Use image upload composable with 'gift-types' folder
const { uploadImage } = useImageUpload('gift-types');
...
const handleImageUpload = async (event) => {
  ...
  try {
    ...
    const fullImageUrl = window.location.origin + '/storage/gift-types/' + res.url;
...
const load = async () => {
  loading.value = true;
  try {
    const res = await giftTypeService.get(route.params.id);
    const giftType = res.data.data;
    Object.assign(form, giftType);
    
    // Set image preview using the accessor URL
    if (giftType.image_url) {
      imagePreview.value = giftType.image_url;
      originalImage.value = giftType.image_url;
    }
  } catch (e) {
    error.value = 'Failed to load gift type';
  } finally {
    loading.value = false;
  }
};
...
const submit = async () => {
  ...
  try {
    if (isEdit.value) {
      await giftTypeService.update(route.params.id, form);
    } else {
      await giftTypeService.create(form);
    }
    router.push({ name: 'GiftTypes' });
  } catch (e) {
