<template>
  <div>
    <div class="flex items-center justify-between mb-6">
      ...
      <router-link to="/admin/blogs/create" class="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-lg font-medium transition-colors">
        Add Blog Post
      </router-link>
    </div>
    ...
    <div v-else class="bg-white rounded-lg shadow overflow-hidden">
      <table class="w-full">
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          <tr v-for="item in blogs.data" :key="item.id" class="hover:bg-gray-50 transition-colors">
            ...
            <td class="px-6 py-4">
              <div class="text-sm text-gray-600">{{ item.author || 'N/A' }}</div>
            </td>
            <td class="px-6 py-4 whitespace-nowrap">
              <span
    ...
    <div class="mt-6 flex items-center justify-between">
      <div class="flex gap-2">
        <button 
          ...
          @click="changePage(blogs.current_page - 1)" 
          :class="blogs.prev_page_url ? 'bg-gray-100 hover:bg-gray-200 text-gray-700' : 'bg-gray-50 text-gray-400 cursor-not-allowed'"
        ...
        <button 
          ...
          @click="changePage(blogs.current_page + 1)" 
          :class="blogs.next_page_url ? 'bg-gray-100 hover:bg-gray-200 text-gray-700' : 'bg-gray-50 text-gray-400 cursor-not-allowed'"
          class="px-4 py-2 rounded-lg font-medium transition-colors"
        >
          Next
        </button>
...
const fetchBlogs = async (page = 1) => {
  loading.value = true;
  try {
    const res = await blogService.list({ page, search: search.value });
    Object.assign(blogs, res.data.data);
  } catch (e) {
    console.error(e);
  } finally {
    loading.value = false;
...
const changePage = (page) => {
  if (page < 1 || page > blogs.last_page) return;
  fetchBlogs(page);
};
