<template>
  <div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 mb-4">
      ...
      <div v-else-if="categories.data.length === 0" class="p-8 text-center text-gray-500">
        No categories found
      </div>
      ...
      <div v-else class="overflow-x-auto">
        <table class="w-full">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="item in categories.data" :key="item.id" class="hover:bg-gray-50 transition-colors">
      ...
      <div v-if="categories.data.length > 0" class="p-4 border-t border-gray-200 flex items-center justify-between">
...
const fetchCategories = async (page = 1) => {
  loading.value = true;
  try {
    const res = await categoryService.list({ page, search: search.value });
...
const deleteCategory = async (id) => {
  try {
    ...
    fetchCategories(categories.current_page);
  } catch (e) {
    alert('Delete failed');
  }
};
